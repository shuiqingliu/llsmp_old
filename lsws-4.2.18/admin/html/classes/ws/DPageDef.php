...
class DFileSect
{
	...
	function DFileSect($tids, $layerId=NULL, $dataLoc=NULL, $holderIndex=NULL)
	{
		$this->_tids = $tids;
...
class DPageDef
{
	...
	function &getPageDef($type, $pid)
	{
		return $this->_pageDef[$type][$pid];
	}

	function &getTabDef($type)
	{
		return $this->_tabDef[$type];
	}

	function add_FilePage_serv()
	{
		$pages = array();
		$pages[] = new DFileSect(
			array(
	...
	function getPageTids($type, $page)
	{
		$pages = &$this->getPageDef($type);
		return $pages[$page];
	}
