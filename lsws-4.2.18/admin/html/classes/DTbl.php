...
require_once('DAttr.php');
require_once('DTblTips.php');
...
class DTbl
{
	...
	var $_width = "100%";
	var $_align;
	var $_isMulti;//0:regular, 1:multi-top, 2:multi-detail
	var $_icon;
	var $_addTbl;
	var $_cols;
	var $_hasB = false;
	...
	var	$_holderIndex = NULL;

	var $_subTbls = NULL;
	var $_defaultExtract = NULL;

	var $_customizedScript;

	function DTbl($id, $title=NULL, $isMulti=0, $addTbl=NULL, $align=0, $icon = NULL)
    {
		$this->_id = $id;
		$this->_title = $title;
	...
	function setRepeated( $holderIndex )
	{
		$this->_holderIndex = $holderIndex;
	}

	function setAttr(&$attrs, $dataLoc=NULL, $layerId=NULL)
	{
		$this->_dattrs = &$attrs;
	...
	function setDataLoc($dataLoc, $layerId=NULL)
	{
		$this->_dataLoc = $dataLoc;
		if ($layerId != NULL)
			$this->_layerId = $layerId;
	...
	function getActionLink($disp, $actions, $editTid='', $editRef='')
	{
		$buf = '';
	...
	function printHtml(&$data, $ref, &$disp)
	{
	...
	function printHeader(&$disp, $actString)
	{
		...
		// tooltip
		$table_help = ' ';
		$dhelp = &DATTR_HELP::singleton();
		if(isset($dhelp->db[$this->_helpKey])) {
			$dhelp_item = $dhelp->db[$this->_helpKey];
			$table_help = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $dhelp_item->render($this->_helpKey);
		}
		else if (count($this->_dattrs) == 1) {
			$av = array_values($this->_dattrs);
			$a0 = $av[0];
			if (strncmp($a0->_inputType, 'textarea', 8) == 0 && isset($dhelp->db[$a0->_helpKey])) {
				$is_blocked = $a0->blockedVersion();
				$version = $is_blocked? $a0->_version: 0;
				$dhelp_item = $dhelp->db[$a0->_helpKey];
	...
	function printViewHtml(&$data, $ref, &$disp)
	{
	...
	function printEditHtml(&$data, &$disp)
	{
	...
	function printHtmlLine(&$data, &$disp, $index)
	{
		$valwid = 0;
		...
		if ( $attr->_type == 'sel1' ) {
			$this->get_sel1_options($disp->_info, $data, $attr);
		}
		...
		if ( $attr->_label ) {
			...
			//add tooltip
			$dhelp = &DATTR_HELP::singleton();
			if(isset($dhelp->db[$attr->_helpKey])) {
				$dhelp_item = $dhelp->db[$attr->_helpKey];
	...
	function printHtmlInputLine(&$data, &$info, $key)
	{
		$attr = &$this->_dattrs[$key];
		...
		$version = $is_blocked? $attr->_version: 0;
		if ( $attr->_type == 'sel1' ) {
			$this->get_sel1_options($info, $data, $attr);
		}
	...
	function printHtmlLine1(&$data, $key0, &$disp, $action_attr)
	{
		static $htmlid;
		...
		if ($action_attr != NULL) {
			...
			if ( is_array($action_attr->_minVal) )
			{
				...
				$ti = $action_attr->_minVal[$data[$index]->_v];
				if ( $ti == NULL )
		...
		foreach( $keys as $key )
		{
			...
			if($key == 0 && $this->_icon != NULL) {
				if($attr->_key == "type" &&  is_array($attr->_maxVal) && is_array($this->_icon)) {
					...
					if(array_key_exists($data['type']->_v, $this->_icon)) {
						$icon_name = $this->_icon[$data['type']->_v];
					}
					else {
				...
				if(array_key_exists('note',$data) && strlen($data['note']->_v)) {
					$buf .= "<td class=icon width=10 align=center><img class='xtip-hover-info{$htmlid}' src='/static/images/icons/info.gif'>";
					$buf .= '<div id="xtip-note-info' . $htmlid . '" class="snp-mouseoffset notedefault">' . $data['note']->_v
						. '</div></td>';
				}
				else {
					$buf .= "<td>&nbsp;</td>";
	...
	function &get_sel1_options(&$info, &$data, &$attr)
	{
		$options = array();
		if ( $attr->_allowNull ) {
			$options[''] = '';
